...
def aboutus_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        eval('''aboutus.objects.filter({}='{}').update({}='{}')'''.format(username_str, username, password_str,
                                                                          init_pwd))
...
def aboutus_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __isAdmin__ == "是" and 'aboutus' != 'forum':
            ...
            if tablename != "users" and 'aboutus'[:7] != 'discuss' and "userid" in aboutus.getallcolumn(aboutus,
                                                                                                        aboutus):
                req_dict["userid"] = request.session.get("params").get("id")
...
def aboutus_save(request):
    ...
    后台新增
    '''
    request.funname = __name__ + "." + aboutus_save.__name__
    request.operation = "新增关于我们"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def aboutus_add(request):
    ...
    前台新增
    '''
    request.funname = __name__ + "." + aboutus_add.__name__
    request.operation = "新增关于我们"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def aboutus_update(request):
    '''
    '''
    request.funname = __name__ + "." + aboutus_update.__name__
    request.operation = "更新关于我们"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def aboutus_delete(request):
    ...
    批量删除
    '''
    request.funname = __name__ + "." + aboutus_delete.__name__
    request.operation = "删除关于我们"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def aboutus_importExcel(request):
    request.funname = __name__ + "." + aboutus_importExcel.__name__
    request.operation = "导入关于我们"
    if request.method in ["POST", "GET"]:
...
def aboutus_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM aboutus {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d')".format(
                xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM aboutus {2} GROUP BY DATE_FORMAT({0}, '%Y-%m')".format(
                xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM aboutus {2} GROUP BY DATE_FORMAT({0}, '%Y')".format(
...
def aboutus_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, sum({1}) AS total FROM aboutus {2} GROUP BY {0} LIMIT 10".format(xColumnName, yColumnName,
                                                                                            where)
        func_name = sys._getframe().f_code.co_name
        table_name = func_name.split('_')[0]
...
def aboutus_valueMul(request, xColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = ''
            if timeStatType == '日':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM aboutus {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d') LIMIT 10".format(
                    xColumnName, item, where, '%Y-%m-%d')

            if timeStatType == '月':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM aboutus {2} GROUP BY DATE_FORMAT({0}, '%Y-%m') LIMIT 10".format(
                    xColumnName, item, where, '%Y-%m')

            if timeStatType == '年':
                sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM aboutus {2} GROUP BY DATE_FORMAT({0}, '%Y') LIMIT 10".format(
...
def aboutus_o_valueMul(request, xColumnName):
    if request.method in ["POST", "GET"]:
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = "SELECT {0}, sum({1}) AS total FROM aboutus {2} GROUP BY {0} LIMIT 10".format(xColumnName, item,
                                                                                                where)
            L = []
            cursor = connection.cursor()
            cursor.execute(sql)
